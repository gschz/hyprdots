...
def main():
    ...
    # Re-apply the Hyprland Lua config so changes to user Lua files
    # (e.g. OPACITY_DEFAULT in hyprland.lua) take effect too. Only for the
    # full "all" reload, never for component-only reloads.
    if "all" in set(args.targets):
        if run_cmd(["hyprctl", "reload"]):
            print_log("-sec", "hyde", "Hyprland config reloaded")
        else:
            print_log("-warn", "hyprctl reload failed; check Hyprland is running")
